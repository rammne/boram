...
# Updated paths to reflect the new PanelContainer hierarchy
...
@onready var keyboard_sfx: AudioStreamPlayer2D = $KeyboardSFX

@export var penalty_percentage: float = 0.10 
...
func _ready() -> void:
	...
	bg_style.bg_color = Color.WEB_GRAY
...
func _process(delta: float) -> void:
	...
	if progress_bar.max_value > 0:
		...
		fill_style.bg_color = Color.DARK_RED.lerp(Color.GREEN, ratio)
...
func _update_display() -> void:
	...
	# Italics tags [i] injected into the BBCode wrappers
	if has_error:
		var error_char: String = target_text[current_index]
		var untyped_portion: String = target_text.substr(current_index + 1)
		label.text = "[center][i][color=green]" + typed_portion + "[/color][color=red]" + error_char + "[/color]" + untyped_portion + "[/i][/center]"
	else:
		var untyped_portion: String = target_text.substr(current_index)
		label.text = "[center][i][color=green]" + typed_portion + "[/color]" + untyped_portion + "[/i][/center]"
